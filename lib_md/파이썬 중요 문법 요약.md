# 알고리즘 구현 문제로 파이썬 익숙해져라. (프로젝트를 해봐도 좋다)
...
print(f"{name} 버전: {version}")
print(f"다음 버전: {version + 0.01}") # 수식 사용 가능
...
2.enumerate() (인덱스와 값을 동시에):
fruits = ["사과", "바나나", "체리"]
for idx, fruit in enumerate(fruits):
    print(idx, fruit)  # 0 사과, 1 바나나 ... (range(len()) 안 써도 됨)

3.zip() (여러 리스트를 병렬로 묶기):
names = ["Kim", "Lee"]
scores = [100, 85]
for name, score in zip(names, scores):
    print(name, score)  # Kim 100, Lee 85

4.가변 인자(*args, **kwargs)
함수 정의부를 볼 때 *나 **가 붙어있어서 당황하기 쉬운 문법입니다.

*args (Tuple 형태로 들어옴): 개수가 정해지지 않은 여러 개의 일반 인자를 받음
**kwargs (Dict 형태로 들어옴): 개수가 정해지지 않은 키-값(Key-Word) 인자를 받음
...
try:
    ...
    print("숫자로 변환할 수 없습니다.") # 에러 발생 시
else:
    print("성공적으로 변환됨!")        # 에러가 안 났을 때만 실행
finally:
    print("무조건 실행되는 마무리 작업") # 에러 여부와 상관없이 무조건 실행

6.람다 함수 (lambda) & map, filter
한 번 쓰고 버릴 간단한 익명 함수를 만들 때 씁니다.
>lambda [파라미터]: [반환할 표현식]

add = lambda a, b: a + b
print(add(3, 5)) # 8

6. map 함수(list로 감싸서 사용)
> map()은 데이터 목록의 모든 요소에 동일한 함수를 적용하여 새로운 결과로 변환해 줍니다.

def square(x):
    return x ** 2
numbers = [1, 2, 3, 4, 5]
# map 객체가 반환되므로 보통 list()로 감싸서 사용합니다.
result = list(map(square, numbers))
print(result)  # 출력: [1, 4, 9, 16, 25]

7. filter 함수 (list로 감싸서 사용)
filter()는 데이터 목록에서 조건식의 결과가 True인 요소만 남기고 나머지는 버립니다.
